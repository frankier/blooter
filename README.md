...
A Bluetooth Classic **HID device emulator** — it makes a Linux machine appear
...
These match the original and must be arranged before running:
...
- For best host compatibility, advertise a keyboard-like device class on the
  adapter (e.g. `0x000540`) — blooter sets this during adapter setup, along with
  making the adapter pairable + discoverable (restored on exit) and registering a
  pairing agent (unless `-n`). The interactive host menu runs alongside accepting
...
```
blooter [-h|-?|--help] [-s|--skipsdp] [-e<NUM>]... [-f<NAME>] [-l] [-x] [-c<FILE>] [-d]
```
...
| `-s`, `--skipsdp` | Skip D-Bus profile/SDP registration (debugging only). |
...
3. Reads Linux input events, translates them into HID Boot mouse (report ID 1)
   ...
   4, …) per forwarded controller, and sends them on the interrupt channel.
...
keyboard-LED output handling. blooter can also **initiate** an outgoing HID
connection to a known host (reconnect-initiate) when a target is set via the
host menu or `[connection] reconnect`; otherwise it only accepts. Pairing is
handled by a built-in agent (`[connection] pairing`). See
